...
class AbilityQueryControllerTest extends RestDocumentTest {
    @MockBean private AbilityQueryService abilityQueryService;

    @Test
    @DisplayName("staffId를 통한 상세조회를 수행하는가")
    void successGetByStaffId() throws Exception {
        // given
        when(abilityQueryService.findAllByStaffId(any()))
                .thenReturn(
                        List.of(
        ...
        // when
        ResultActions perform = mockMvc.perform(get("/abilities").param("staff-id", "3"));
        // then
    ...
    void successGetSummaryByStaffId() throws Exception {
        // given
        when(abilityQueryService.findSummaryByStaffId(any()))
                .thenReturn(new AbilitySummary(StaffSummaryFixture.THOR.toDto(), pointExample()));
        // when
        ResultActions perform = mockMvc.perform(get("/abilities/summary").param("staff-id", "3"));
        // then
        perform.andExpect(status().isOk());
        ...
        perform.andDo(print())
                .andDo(
                        document(
                                "get ability summary by staff id",
                                getDocumentRequest(),
                                getDocumentResponse()));
    }
    ...
    void successGetMyAbility() throws Exception {
        // given
        when(abilityQueryService.findAllMyAbility(any()))
                .thenReturn(
                        List.of(
                                AbilityResponseFixture.SAMPLE1.toDto(),
                                AbilityResponseFixture.SAMPLE2.toDto(),
                                AbilityResponseFixture.SAMPLE3.toDto()));
        // when
        ResultActions perform = mockMvc.perform(get("/abilities/myability").param("team-id", "3"));
        // then
        perform.andExpect(status().isOk());
    ...
    void successGetMyAbilitySummary() throws Exception {
        // given
        when(abilityQueryService.findAllMyAbilitySummary(any()))
                .thenReturn(new AbilitySummary(StaffSummaryFixture.THOR.toDto(), pointExample()));
        // when
        ResultActions perform =
                mockMvc.perform(get("/abilities/myability/summary").param("team-id", "3"));
        // then
        perform.andExpect(status().isOk());
