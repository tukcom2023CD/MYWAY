...
class StaffServiceTest extends LoginTest {
    ...
    @Autowired
    private EntityQueryService<Team, Long> teamService;
    @Autowired
    ...
    class CallCreateProjectLeaderStaff {
        ...
        void successCreate() throws Exception {
            ...
            //then
            Staff result = staffQueryRepository.findAll().stream().filter(s -> !before.contains(s)).findFirst().get();
    ...
    void successInvite() throws Exception {
        //given
        InviteRequest req = new InviteRequest(TEAM1.getId(), Set.of(MEMBER1.getId(), MEMBER2.getId()));
        List<Staff> before = staffQueryRepository.findAll();
        //when
        ...
        List<Staff> after = staffQueryRepository.findAll().stream().filter(s -> !before.contains(s)).toList();
