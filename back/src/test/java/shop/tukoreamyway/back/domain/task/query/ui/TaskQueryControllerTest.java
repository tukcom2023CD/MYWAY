...
class TaskQueryControllerTest extends RestDocumentTest {
    ...
    void successGetAllByCondition() throws Exception {
        //given
        TaskSearchCondition condition = new TaskSearchCondition(TaskStatus.PROGRESS, AbilityCategory.DEVELOPMENT, 1L, 2L);
        when(taskQueryService.findAllByCondition(condition)).thenReturn(
                List.of(
        ...
        //when
        ResultActions perform = mockMvc.perform(get("/tasks")
        ...
        // docs
        perform.andDo(print())
                .andDo(document("get task list by condition", getDocumentRequest(), getDocumentResponse()));
    ...
    void successGetAllMyTask() throws Exception {
        //given
        MyTaskCondition condition = new MyTaskCondition(AbilityCategory.DEVELOPMENT, TaskStatus.PROGRESS, 1L, 2L);
        when(taskQueryService.findAllMyTask(condition)).thenReturn(
                List.of(
                        TaskSummaryFixture.SAMPLE1.toDto(),
                        TaskSummaryFixture.SAMPLE2.toDto(),
                        TaskSummaryFixture.SAMPLE3.toDto()
        ...
        //when
        ResultActions perform = mockMvc.perform(get("/tasks/mytask")
                .param("status", "PROGRESS")
                .param("category", "DEVELOPMENT")
                .param("sprintId", "1")
                .param("teamId", "2"));
        ...
        // docs
        perform.andDo(print())
                .andDo(document("get my task list by condition", getDocumentRequest(), getDocumentResponse()));
    ...
    void successGetById() throws Exception {
        //given
        when(taskQueryService.findById(1L)).thenReturn(
                new TaskResponse(1L, "요약요약", "설명", TaskStatus.READY.getName(), 4, AbilityCategory.DEVELOPMENT.getName(), SprintResponseFixture.SAMPLE1.toDto(), StaffSummaryFixture.CAPTAIN.toDto(), StaffSummaryFixture.THOR.toDto())
        );
        //when
