...
class QuestionQueryControllerTest extends RestDocumentTest {
    ...
    void successGetAllByWriterId() throws Exception {
        ...
        //given
        when(questionQueryService.findAllByWriterId(any())).thenReturn(
                List.of(
                        new QuestionResponse(2L, "제목", "내용", 5L, List.of("태그1", "태그2"),writer),
                        new QuestionResponse(3L, "제목", "내용", 5L, List.of("태그1", "태그2"),writer),
                        new QuestionResponse(4L, "제목", "내용", 5L, List.of("태그1", "태그2"),writer)
                )
        );
        //when
        ResultActions perform =
                mockMvc.perform(
                        RestDocumentationRequestBuilders.get(
                                "/questions").param("writer-id", writer.getId().toString()));
        ...
        // docs
        perform.andDo(print())
                .andDo(document("get question list By writer id", getDocumentRequest(), getDocumentResponse()));
