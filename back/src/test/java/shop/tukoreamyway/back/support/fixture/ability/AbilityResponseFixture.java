...
public enum AbilityResponseFixture {
    SAMPLE1(1L, AbilityCategory.DEVELOPMENT, StaffSummaryFixture.THOR.toDto(), 10L, LocalDateTime.now(),GrantLocation.DO_TASK,null),
    SAMPLE2(2L, AbilityCategory.DEVELOPMENT, StaffSummaryFixture.THOR.toDto(), 14L, LocalDateTime.now(),GrantLocation.PROJECT_MANAGER_DISCRETION,StaffSummaryFixture.CAPTAIN.toDto()),
    SAMPLE3(3L, AbilityCategory.DEVELOPMENT, StaffSummaryFixture.THOR.toDto(), 16L, LocalDateTime.now(),GrantLocation.DO_TASK,null),;
    ...
    public AbilityResponse toDto() {
        return new AbilityResponse(id, category.getName(), receiver, point, grantedAt, grantLocation.name(), grantor);
    }
}
